...
<script>
/**
 * Function to initialize the DataTable for the 'list' table and set up delete_project event.
 */
$(document).ready(function () {
    // Initialize the DataTable for the 'list' table
    $('#list').dataTable();

    // Set up click event for elements with the class 'delete_project'
    $('.delete_project').click(function () {
        // Display confirmation dialog and execute delete_project function with project ID as parameter
        _conf("Are you sure to delete this project?", "delete_project", [$(this).attr('data-id')]);
    });
});

/**
 * Function to delete a project with the specified ID.
...
function delete_project($id) {
    ...
    // Use AJAX to send a request to delete the project with the specified ID
    $.ajax({
        url: '/task-management-system/controller/ajax.php?action=delete_project',
        method: 'POST',
        data: { id: $id },
        success: function (resp) {
            // If the response is 1, display success toast and reload the page after a delay
            if (resp == 1) {
