 ...
 <div class="col-12">
  ...
  <hr>
    ...
    <?php
        ...
        // Check if the user type is 2 (Project Manager)
        if ($_SESSION['login_type'] == 2) {
            // Set the WHERE clause for Project Manager
            $where = " WHERE manager_id = '{$_SESSION['login_id']}' ";
        }
        // Check if the user type is 3 (Regular Employee)
        elseif ($_SESSION['login_type'] == 3) {
            // Set the WHERE clause for Regular Employee using user_ids
            $where = " WHERE CONCAT('[', REPLACE(user_ids, ',', ']['), ']') LIKE '%[{$_SESSION['login_id']}]%' ";
        }

        // Initialize the $where2 variable
        $where2 = "";

        // Check if the user type is 2 (Project Manager)
        if ($_SESSION['login_type'] == 2) {
            // Set the WHERE clause for Project Manager in a different context (possibly another table alias 'p')
            $where2 = " WHERE p.manager_id = '{$_SESSION['login_id']}' ";
        }
        // Check if the user type is 3 (Regular Employee)
        elseif ($_SESSION['login_type'] == 3) {
            // Set the WHERE clause for Regular Employee in a different context (possibly another table alias 'p') using user_ids
